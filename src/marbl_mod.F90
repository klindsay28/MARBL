...
!------------------------------------------------------------------------
  ...
  subroutine marbl_set_surface_forcing( &
    ...
    real (r8)          :: co3_tmp(num_elements)          ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: co2star_tmp(num_elements)      ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: dco2star_tmp(num_elements)     ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: dco2star_pos_inc(num_elements) ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: dco2star_neg_inc(num_elements) ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: pco2surf_tmp(num_elements)     ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: dpco2_tmp(num_elements)        ! temporary, for computing sflux shadow fluxes and derivatives
    real (r8)          :: ph_tmp(num_elements)           ! temporary, for computing sflux shadow fluxes and derivatives
    ...
    if (lflux_gas_o2 .or. lflux_gas_co2) then
       ...
       if (lflux_gas_co2) then
          ...
          !-----------------------------------------------------------------------
          !  Set flux_co2_shadow, and its derivatives, if NK shadow tracers are enabled
          ...
          if (lNK_shadow_tracers) then
            ...
            flux_co2_shadow(:) = pv_co2(:) * dco2star_tmp(:)

            stf(:, dic_shadow_ind)  = stf(:, dic_shadow_ind) + flux_co2_shadow(:)

            ! centered finite difference approximation wrt dic_shadow
            ...
            ! centered finite difference approximation wrt alk_shadow
